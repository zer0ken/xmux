...
- A console is one PTY child owned by whoever spawned it rather than by the registry, for
  a program that talks to a terminal and nothing else. It shares the PTY mechanics with an
  ...
  its spawner sees through to an exit code. What it hands back is the bytes the child
  wrote, in order, because a caller that must RECOGNISE something in the stream needs the
  stream; the grid it keeps is its own, and answers the child's terminal queries.
