...
cockpit folds into `State`. It is a METADATA channel only — the per-session PTY
...
right `HostEvent` is emitted. The cockpit reads the inventory to (re)build the
tree and folds `HostEvent`s through `State::apply_event`.

## Module Seams

- `HostManager::ensure` spawns the `-CC` control child with an argv composed
  across the two orthogonal axes — `Transport::control_argv(&Backend::control_argv())`
...
- `HostEvent` is the outbound vocabulary consumed by `State::apply_event`; the
  cockpit's `run_event_effect` runs the returned `EventEffect`s back against these
...
- The control argv is composed from the transport and backend axes; no mux verb
...
- Do not block: the reader and writer run on their own threads and communicate
  with the cockpit loop over channels.
...
- Run `host` tests (manager ensure/reap idempotency) and the cockpit/state tests
