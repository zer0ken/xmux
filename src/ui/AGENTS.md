...
- A rebuild holds the selection on its session whenever that session survives the
  ...
  the mux moves it.
- The nav's two navigation axes name the two things its list is made of: the vertical
  one walks the cards, the horizontal one walks the categories, landing on a category's
  first card. A category is a source with sessions to show, or the whole host band at
  once. Neither axis is defined by where a card sits on screen, so both mean the same
  thing in the side column and in the portrait band.
