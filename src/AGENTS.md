...
the app event loop, host metadata management, display attachment spawning, the
control socket, and the app / ui / model / mux / machine / state submodules.

## Mental Model

The app is the coordinator. It receives stdin, control socket commands, host
...
- `driver.rs` holds ONLY the mux-agnostic display seam: the `MuxDriver` trait,
  ...
  window-selection helper, and the thin wrapper resolving a host's own driver.
  ...
  server model exists. Showing a session carries the per-host display
...
- `host/` owns control-mode reader/writer machinery, poll task management, host
  inventory, and host events. It is a metadata path only. Spawning the
...
- The runtime coordinates these modules and owns the main event loop. Inbound
  host events route through the event-driven mutation site on the state; the
  handler is then a thin executor running the returned effects against the host
  clients, the registry, and the display worker.
- A source is a thin config adapter: an alias, a mux binary, a machine kind, an
  injectable runner, and the assembly of a value host for the off-loop and CLI
  paths that cannot borrow the event loop's live host. Enumeration, manage
  lifecycle operations, and interactive-attach argv are NOT here: they live on
  the host, mux, and transport, which the adapter reaches by building a host and
  injecting its runner. The machine axis is solely the transport, whose shared
  shell vocabulary lives beside it; the adapter carries no transport-wrapping
  implementation of its own.
- The environment is config assembly plumbing for source definitions and command
  construction, and the ONE place this machine's mux list is resolved. That
  answer is threaded into both source-list and host-registry construction rather
  than re-derived, so source ids and host ids cannot disagree, and resolving it
  ...
  insert into the runtime host registry in the same handler: the environment is
  ...
  operation.
- The roster answers only "which machines does xmux offer as sources": one or
  ...
  empty list rather than an error. Distinct from the machine axis (how a command
  REACHES a machine) and from discovery (scanning a machine for sessions).
...
- The per-mux display decision lives in the driver implementation. The runtime
  does not branch on mux kind for display; it resolves the host's driver, asks
...
- Do not treat the source adapter as the preferred place for new execution
  semantics. Machine execution belongs to the transport, mux vocabulary and
  classification (attach argv, server model, enumeration) to the mux, and
  per-host display orchestration with its concrete switch-or-reattach decision
...
- For app changes, locate the event source and the state it owns before adding
  fields or channels.
- For host or display changes, decide whether the behavior belongs to metadata,
