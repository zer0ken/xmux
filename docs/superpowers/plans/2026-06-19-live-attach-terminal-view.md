...
- **Status-bar clear-leak repaint (spec §4 — NOT yet covered by a dedicated task; implement in Task 12's pump or as a Task 12.5 follow-up):** the foreground child is sized `cols×(rows-1)` so it never addresses the last physical row, EXCEPT a full-screen clear (`ESC[2J` / `ESC[…J`) hits the whole physical screen. The foreground pump must scan each outgoing chunk for a clear sequence (a rolling-tail scan, exactly like `InputMachine::track_paste`'s `paste_scan`) and, when found, re-emit `status_bar_bytes(..)` after the chunk (the bytes already wrap themselves in cursor save/restore `ESC7`/`ESC8`). This needs a new field on the foreground pump path in `spawn_attachment` (a `status_bar: Arc<Mutex<Vec<u8>>>` the cockpit updates on each Passthrough entry, scanned and re-emitted by the owner pump). Add a unit test `pump_repaints_status_bar_after_clear_sequence` driving a rolling-tail scanner over chunked `b"\x1b[2J"` (split across two chunks) and asserting the status bytes are appended. This was deferred from Task 12 to keep that task's diff reviewable; do it before the Task 14 human gate.
