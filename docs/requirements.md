...
- **FR-B7** - A card that is WAITING turns ONE spinner, standing in the first of its
  ...
  its host screen, which has the room to keep a tool's diagnostic whole, while a card
...
- **FR-C3** - Source degradation is graceful, never a silent loss: an unreachable source
  is marked `⚠ unreachable`, and its host screen states why - the reason its transport
