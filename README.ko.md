...
네트워크에는 응답하지만 자격 증명이 거부된 원격 호스트는 `locked`(`?` 표시)로 나타난다.
터미널 뷰의 해당 패널로 포커스를 옮겨 사용자 이름과 마스킹된 비밀번호를 그 안에 입력하면,
xmux가 인증된 연결 하나를 확립해 나머지 세션이 그 연결을 재사용하고, 그 호스트만 다시 조회한다.
검증한 적 없는 키로 응답한 호스트는 `unverified host key`(`!` 표시)로 나타난다. 해당 패널로
포커스를 옮겨 Enter를 누르면 그 키를 수락한다. 비밀번호까지 요구하는 호스트는 그다음
`locked`가 되어 비밀번호를 묻는다. 키가 바뀐 호스트는 unreachable로 남는다. 그것은 xmux
바깥에서 판단할 문제이기 때문이다.
