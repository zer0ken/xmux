...
비밀번호 칸은 선택이다. 제출하면 그 패널에서 ssh가 실제로 실행되고 그 화면이 그대로
보인다. 호스트 키 확인과 비밀번호는 입력한 값으로 대신 답하고, 2단계 인증 코드나 키
암호 문구처럼 그 밖의 프롬프트는 사용자가 직접 입력한다. Esc는 시도를 끝낸다. 연결에
성공하면 인증된 연결 하나가 확립되어 나머지 세션이 그 연결을 재사용하고, 그 호스트만
다시 조회한다. 그다음에 무엇을 할지는 체크박스 둘이 정한다. 입력한 값을
`~/.ssh/config` 스탠자로 적을지, 그리고 공개키를 그 호스트에 등록해 비밀번호를 다시
묻지 않게 할지다.
