...
비밀번호 칸은 선택이다. 제출하면 xmux가 인증된 연결 하나를 확립해 나머지 세션이 그
연결을 재사용하고, 그 호스트만 다시 조회한다. 그다음에 무엇을 할지는 체크박스 둘이
정한다. 입력한 값을 `~/.ssh/config` 스탠자로 적을지, 그리고 공개키를 그 호스트에
등록해 비밀번호를 다시 묻지 않게 할지다.
