...
것이다. 설정해 둔 어느 호스트의 mux 세션이든, 터미널 하나에서 즉시 제자리에서
오간다.

## 기능

- **모든 호스트를 하나의 목록으로.** 세션 하나가 카드 하나, 로컬과 ssh를
  한 화면에 담는다. 호스트는 `~/.ssh/config`와 tailnet에서 알아서 찾아낸다.
...
- **꼭 필요한 곳은 폴링 없이.** tmux 소스는 컨트롤 모드(`-CC`)로 추적한다. psmux와
  zellij 소스는 폴링한다. 둘 다 밀어주는 통로가 없기 때문이다. 어느 쪽이든 목록은
...
하고, 대상으로 삼는 호스트마다 지원되는 mux가 있어야 한다. unix는 `tmux`, Windows는
`psmux`다. 둘은 같은 명령어 언어를 쓴다. `zellij`도 된다. 이쪽은 명령어 언어가 아예
달라서 자기 CLI로 따로 다룬다. 호스트의 mux는 그 바이너리가 스스로 답하는 이름으로
...
xmux doctor                   # 설정과 소스별 접속 가능 여부 점검
...
호스트 하나가 mux를 여러 개 동시에 띄울 수 있고, xmux는 그 각각을 별도 소스로 다룬다.
이 호스트에 관해서는 적을 필요조차 없다. `mux`를 기본값으로 두면 xmux가 지원하는 mux 중
이 컴퓨터에 실제로 깔려 있는 것을 찾아 각각을 소스로 올린다. psmux 옆에 zellij를 깔면
다음 실행부터 그냥 보인다.

특정 조합을 고정하고 싶을 때는 이 호스트든 원격이든 목록으로 적는다.
...
호스트의 소스 이름은 `local:psmux`, `local:zellij`가 되고, 이 이름이 `xmux ls`가 찍고
`xmux send switch`가 받는 이름이다. mux를 하나만 준 호스트는 예전 그대로 맨 이름(`local`,
`prod`)을 지킨다. `exclude`는 호스트를 가리키므로, 하나를 빼면 그 위의 mux가 전부 빠진다.

그 호스트에 깔려 있지 않은 mux를 적었다면 조용히 넘기지 않는다. 그 소스는 mux가 낸 메시지와
...
원격 호스트도 물어보지만, 앱이 뜬 뒤에 물어본다. 원격 probe는 mux마다 ssh 왕복이라 아무것도
그것을 기다리지 않는다. 설정한 소스가 먼저 그려지고, 아무도 적지 않은 mux는 그 호스트가
답하는 대로 몇 초 뒤에 나타난다. 발견은 더하기만 한다. 이미 보여주던 mux의 이름은 그대로
남으므로, 타이핑한 이름이나 고른 세션이 밑에서 바뀌지 않는다.

## 호스트 목록의 출처

**호스트**는 mux를 호스팅하는 머신이면서 xmux가 닿을 수 있는 것이고, 이 목록이 그 범위를
정한다. 호스트 하나가 mux를 여러 개 띄울 수 있으며, 그 짝 하나하나가 **소스**다(위 참고).

모든 제공자가 기본으로 켜져 있다. 그래서 닿을 수 있는 호스트가 곧 xmux가 보여주는
호스트이고, 손으로 맞춰 둘 것이 없다. `~/.ssh/config`의 `Host` 별칭, 이 기계가 속한
tailnet의 온라인 피어, 이 상자의 WSL 배포판, 그리고 `local`이다. 원하지 않는 것은 끄면
된다.
...
wsl = true          # 기본값. 이 상자에 설치된 WSL 배포판
```
...
WSL 배포판은 그 자체로 하나의 호스트다. 그래서 배포판 안에서 도는 tmux도 다른 소스와
똑같다. 목록에 올라오고, 세션이 흘러 들어오고, 그 안으로 들어가는 조작은 원격으로
들어가는 것과 같은 키다. 다른 제공자와 마찬가지로, 이 상자가 가진 배포판이 모두
올라온다.
...
목록 조회를 끄고 하나만 지정할 수도 있고, 이때 mux도 같이 정한다.
...
목록 조회는 `wsl.exe`를 실행하는 일이다. Docker Desktop이 설치된 상자에는 mux를 아예
돌리지 않는 배포판이 있어서 unreachable로만 보인다. 그런 것은 이름으로 빼거나
(`exclude = ["wsl.docker-desktop"]`), 목록 조회를 끄고 쓰려는 배포판 하나만 `[[wsl]]`로
올리면 된다.
...
돌리면 확정된 로컬 mux, ssh 사용 가능 여부, 소스별 접속 가능 여부를 보여준다.
...
xmux는 직교하는 두 축을 중심으로 짜였다. `Mux`(mux별 동작)와 `Transport`(호스트별
실행)다. 이렇게 나눈 덕분에 mux 계열과 호스트 계열이 서로 뒤섞이지 않고 조합된다.
