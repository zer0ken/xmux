...
오른쪽 창에는 선택한 세션의 **실시간 화면**이 표시된다. 목록에서 커서를
움직이면 오른쪽 창이 해당 세션의 화면으로 즉시 전환된다. 세션이 로컬 psmux든
ssh 너머의 tmux든 다른 머신의 zellij든 이 동작은 동일하다.

![xmux의 분할 화면. 왼쪽 목록에 이 머신의 psmux 세션과 WSL 배포판 안의 tmux
세션이 함께 있고, 오른쪽에는 선택한 세션의 실시간 화면이 차 있다.](docs/assets/xmux.png)
...
`zellij`다. xmux는 호스트가 어느 바이너리로 응답하는지를 보고 그 호스트의 mux를
판별한다. 따라서 호스트마다 다른 mux가 설치되어 있어도 설정할 것이 없다.
...
왼쪽 창이 목록이고, 오른쪽 창이 선택한 세션의 실시간 화면이다. 키보드 포커스는
두 창 중 한쪽에만 있다.
...
| `Enter` | 선택한 세션의 실시간 화면으로 포커스를 옮긴다 |
...
`prefix q`는 종료, `prefix ?`는 키 도움말 토글, `prefix Tab`은 목록과 화면
사이의 포커스 이동이다. 마우스 입력도 지원한다. 행을 클릭하면 그 행이
선택되고, 오른쪽 창을 클릭하면 포커스가 그 창으로 옮겨진다. 나머지 키는
[`docs/keybind.md`](docs/keybind.md)에 있다.

## 호스트와 소스

**호스트**는 mux가 동작하고 있고 xmux가 접근할 수 있는 머신이다. xmux는
호스트를 `~/.ssh/config`, tailnet, 이 머신의 WSL 배포판, 그리고 `local`에서
자동으로 발견한다. 호스트 하나가 여러 mux를 동시에 제공할 수 있으므로, psmux와
zellij가 함께 동작하는 호스트는 둘 다 노출한다. 호스트와 mux의 조합
